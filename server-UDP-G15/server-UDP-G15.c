...
int main() {
    ...
    while (1) {
        ...
        int bytes_received = recvfrom(server_socket, input_buffer, MAX_BUFFER_SIZE, 0,
        ...
        if (getnameinfo((struct sockaddr*)&client_address, client_address_len,
                        client_hostname, sizeof(client_hostname), NULL, 0, 0) != 0) {
        ...
        if (strcmp(input_buffer, "Hello") != 0) {
            ...
            sendto(server_socket, output_buffer, strlen(output_buffer), 0,
                   (struct sockaddr*)&client_address, client_address_len);
        }
    }
