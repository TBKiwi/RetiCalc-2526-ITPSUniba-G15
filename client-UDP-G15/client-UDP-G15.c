...
int main() {
    ...
    sendto(client_socket,handshake_message,strlen(handshake_message),0,(struct sockaddr*)&server_address,sizeof(server_address));
    ...
    sendto(client_socket,message_buffer,strlen(message_buffer),0,(struct sockaddr*)&server_address,sizeof(server_address));

    struct sockaddr_in sender_address;
    socklen_t sender_address_len = sizeof(sender_address);

    int bytes_received = recvfrom(client_socket,message_buffer,BUFFER_MAX,0,(struct sockaddr*)&sender_address,&sender_address_len);
    ...
    if (bytes_received > 0) {
        ...
        if (getnameinfo((struct sockaddr*)&sender_address,sender_address_len,server_hostname,sizeof(server_hostname),NULL,0,0) != 0) {
